...
import { generateDemoActivities } from './data/sampleActivities'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function checkCache() {
      ...
      if (cached && cached.length > 0) {
        ...
        if (consent.weather && needsWeather.length > 0) enrichWithWeather(cached, { persist: true })

        const needsGeo = cached.filter(a => !a.startLocation && a.startLat != null && a.startLon != null)
        if (consent.geocoding && needsGeo.length > 0) enrichWithGeocoding(cached, { persist: true })
      }
    }
    checkCache()
  ...
  async function handleImportComplete(parsedActivities) {
    setIsDemo(false)
    await setActivities(parsedActivities)

    setAppState('dashboard')

    const needsWeather = parsedActivities.filter(a => !a.weather && a.startLat != null && a.startLon != null)
    if (consent.weather && needsWeather.length > 0) enrichWithWeather(parsedActivities, { persist: true })

    const needsGeo = parsedActivities.filter(a => !a.startLocation && a.startLat != null && a.startLon != null)
    if (consent.geocoding && needsGeo.length > 0) enrichWithGeocoding(parsedActivities, { persist: true })
  }

  function handleTryDemo() {
    setActivitiesInMemory(generateDemoActivities())
    setAppState('dashboard')
    setIsDemo(true)
  ...
  function handleLeaveDemoUpload() {
    setActivitiesInMemory([])
    setAppState('empty')
    setIsDemo(false)
    setSelectedActivity(null)
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 flex-1 w-full">
        ...
        {appState === 'dashboard' && (
          <>
          {isDemo && (
            <DemoBanner onUploadOwn={handleLeaveDemoUpload} />
          )}
